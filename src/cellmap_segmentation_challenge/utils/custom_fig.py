...
def get_fig_dict(
    ...
    for c, label in enumerate(classes):
        ...
        #     batch_size,
        #     6 + colorbar,
        #     figsize=(fig_size * (6 + colorbar * colorbar_size), fig_size * batch_size),
        #     gridspec_kw=grid_spec_kw,
        # )
        ncols = 4 + int(colorbar)  
        fig, ax = plt.subplots(
    batch_size, ncols,
    figsize=(fig_size * sum(grid_spec_kw.get("width_ratios", [1]*ncols)),
             fig_size * batch_size),
    gridspec_kw=grid_spec_kw,
)
        ...
        for b in range(batch_size):
            ...
            # low‐range prediction (0–30)
            im_low = ax[b, 3].imshow(
                output,
                clim = (-10,20)
            )
            ax[b, 3].axis("off")
            ...
            cbar_low = fig.colorbar(im_low, cax=ax[b, 4], orientation="vertical")
            ax[b, 4].set_title("Intensity")

            # high‐range  (→ **note**: use index 5 for the image)
            # im_high = ax[b, 5].imshow(output, clim=(20,100))
            # ax[b, 5].axis("off")
            # ax[b, 5].set_title(f"Pred {label}\n30–100")
            # cbar_high = fig.colorbar(im_high, cax=ax[b, 6], orientation="vertical")
            # ax[b, 6].set_title("Intensity")
            # slide your colorbar(s) one column to the right:
            ...
            if y_pad <= 0:
                ...
                y_slice = slice(y_pad, -y_pad)
            # ax[b, 1].imshow(input_img[x_slice, y_slice], cmap="gray", clim=clim)
            ax[b, 1].imshow(input_img[x_slice, y_slice], cmap="gray")
            ax[b, 1].axis("off")
            ax[b, 1].set_title("Raw")
            # ax[b, 0].imshow(input_img, cmap="gray", clim=clim)
            ax[b, 0].imshow(input_img, cmap="gray")
            ax[b, 0].axis("off")
            ax[b, 0].set_title("Full FOV")
            w, h = output.shape[1], output.shape[0]
