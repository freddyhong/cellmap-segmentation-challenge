...
class NestedUpBlock2D(nn.Module):
    """Upsampling block for 2D UNet++ with dense skip connections"""
    def __init__(self, in_channels, skip_channels, out_channels, bilinear=True):
        ...
        self.conv = DoubleConv2D(total_in_channels, out_channels)

    def forward(self, x_up, skip_connections: List[torch.Tensor]):
        x_up = self.up(x_up)
        if self.reduce_channels is not None:
            x_up = self.reduce_channels(x_up)

        if skip_connections:
            # Match size if needed (e.g., due to non-padded convolutions in encoder)
            target = skip_connections[0]
            diffY = target.size()[2] - x_up.size()[2]
            diffX = target.size()[3] - x_up.size()[3]
...
class UNet2DPlusPlus(nn.Module):
    """
    A 2D implementation of UNet++.
    """
    def __init__(self, n_channels: int, n_classes: int, deep_supervision: bool = False, bilinear: bool = True):
        super(UNet2DPlusPlus, self).__init__()
        ...
        self.conv0_0 = DoubleConv2D(n_channels, 32)
        self.conv1_0 = DoubleConv2D(32, 64)
        self.conv2_0 = DoubleConv2D(64, 128)
        self.conv3_0 = DoubleConv2D(128, 256)
        self.conv4_0 = DoubleConv2D(256, 512)
        
        # Nested Skip Pathways (X^i,j)
        # j=1
        self.conv0_1 = NestedUpBlock2D(64, 32, 32, bilinear)
        self.conv1_1 = NestedUpBlock2D(128, 64, 64, bilinear)
        self.conv2_1 = NestedUpBlock2D(256, 128, 128, bilinear)
        self.conv3_1 = NestedUpBlock2D(512, 256, 256, bilinear)
        
        # j=2
        self.conv0_2 = NestedUpBlock2D(64, 32+32, 32, bilinear)
        self.conv1_2 = NestedUpBlock2D(128, 64+64, 64, bilinear)
        self.conv2_2 = NestedUpBlock2D(256, 128+128, 128, bilinear)
        
        # j=3
        self.conv0_3 = NestedUpBlock2D(64, 32+32+32, 32, bilinear)
        self.conv1_3 = NestedUpBlock2D(128, 64+64+64, 64, bilinear)
        
        # j=4
        self.conv0_4 = NestedUpBlock2D(64, 32+32+32+32, 32, bilinear)
        ...
        if self.deep_supervision:
            self.out1 = nn.Conv2d(32, n_classes, kernel_size=1)
            self.out2 = nn.Conv2d(32, n_classes, kernel_size=1)
            self.out3 = nn.Conv2d(32, n_classes, kernel_size=1)
            self.out4 = nn.Conv2d(32, n_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            ...
            # Return only the final, most refined output
